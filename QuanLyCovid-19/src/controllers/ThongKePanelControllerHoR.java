 ...
 * @author Hai
 */
public class ThongKePanelControllerHoR {
...
//    private JTextField denNamJtf;
    ...
    private final String[] COLUMNS = {"ID", "Họ tên", "Ngày sinh", "Giới tính","Địa chỉ hiện nay","Có liên quan đến COVID-19?"};

    public ThongKePanelControllerHoR(JComboBox genderJcb, JComboBox statusJcb, JTextField tuTuoiJtf, JTextField denTuoiJtf, JPanel jpnView) {
...
//    public ThongKePanelController(JComboBox<String> GenderJcb, JComboBox<String> StatusJcb, JTextField tuTuoiJtf, JTextField denTuoiJtf, JPanel tableJpn) {
...
//        int denNam = 2100;
        ...
        int x = this.StatusJcb.getSelectedIndex();
        String status;
        if(x == 0) status = "";
        else if(x == 1) status = "có";
        else status = "không";
        //JOptionPane.showMessageDialog(null,x +" "+ status);
        try {
...
//                denNam = Integer.parseInt(this.denNamJtf.getText().trim());
        ...
        this.listNhanKhauBeans = this.nhanKhauService.statisticNhanKhau(tuTuoi, denTuoi, gender, status);
        setDataTable();
    }
